...
def apply(grayscale, img,tshirt):
...
def detection(grayscale, img):
    #detect pal, inc count%totoal tshirts, call apply wiith tshirt
      
    hand_cascade = cv2.CascadeClassifier('palm.xml')
    hand= hand_cascade.detectMultiScale(grayscale, 1.3, 5)
    
    if len(hand):
        #change tshirt
        global tshirtno
        #print("hand dertected")
        tshirtno= (tshirtno+1)%size
     
    #transparet tshirt else tshirt2
    tshirt= cv2.imread("tshirt"+str(tshirtno)+".png", -1)        
    #cv2.imshow("tshirt preview",img)        
    return apply(grayscale, img, tshirt)     
